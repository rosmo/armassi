...
try:
    ...
    from . import copy
    from collections import namedtuple
    # Not supporting unicode now due to Circuitpython limitations
    def wcwidth(t):
        if ord(t) < 32:
            return -1
        return 1
...
Savepoint = namedtuple("Savepoint", ["cursor", "charset", "origin", "wrap"])

class Char:
    def __init__(self, data=None, bold=False, reverse=False, blink=False, fg=1, bg=0):
        self.data = data
        self.bold = bold
    ...
    def __copy__(self):
        return Char(data=self.data, bold=self.bold, reverse=self.reverse, fg=self.fg, bg=self.bg)
...
class Screen:
    ...
    def default_char(self):
        ...
        return Char(data=" ", fg=1, bg=0, reverse=False)
